...
from frontend.g2pw.dataset import get_char_phoneme_labels
from frontend.g2pw.dataset import get_phoneme_labels
from frontend.g2pw.dataset import prepare_onnx_input
from frontend.g2pw.utils import load_config
from frontend.zh_normalization.char_convert import tranditional_to_simplified
...
class G2PWOnnxConverter:
    def __init__(
        self,
        model_dir: os.PathLike = "resources/g2pW",
        style: str = "pinyin",
        ...
        self.session_g2pW = onnxruntime.InferenceSession(
            os.path.join(g2pW_path, "g2pw.onnx"), sess_options=sess_options
        )
        self.config = load_config(config_path=os.path.join(g2pW_path, "config.py"), use_default=True)

        self.model_source = model_source if model_source else self.config.model_source
        self.enable_opencc = enable_non_tradional_chinese
        self.tokenizer = BertTokenizer.from_pretrained(self.config.model_source)

        polyphonic_chars_path = os.path.join(g2pW_path, "POLYPHONIC_CHARS.txt")
        monophonic_chars_path = os.path.join(g2pW_path, "MONOPHONIC_CHARS.txt")
        self.polyphonic_chars = [
            line.split("\t")
            for line in open(polyphonic_chars_path, encoding="utf-8").read().strip().split("\n")
        ]
        self.non_polyphonic = {
            "一",
            "不",
        ...
        self.monophonic_chars = [
            line.split("\t")
            for line in open(monophonic_chars_path, encoding="utf-8").read().strip().split("\n")
        ]
        ...
        with open(
            os.path.join(g2pW_path, "bopomofo_to_pinyin_wo_tune_dict.json"), "r", encoding="utf-8"
        ) as fr:
        ...
        with open(os.path.join(g2pW_path, "char_bopomofo_dict.json"), "r", encoding="utf-8") as fr:
    ...
    def _prepare_data(self, sentences: List[str]) -> Tuple[List[str], List[int], List[int], List[List[str]]]:
        ...
        for sent_id, sent in enumerate(sentences):
            ...
            for i, char in enumerate(sent_s):
                if char in self.polyphonic_chars_new:
                    texts.append(sent)
                    query_ids.append(i)
